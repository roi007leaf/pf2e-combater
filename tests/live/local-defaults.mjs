...
export const defaultsPath = path.join(homedir(), '.config', 'pf2e-combater', 'live.json');
export const sharedVisionerDefaultsPath = path.join(
  homedir(),
  '.config',
  'pf2e-visioner',
  'live.json',
...
export async function loadLocalDefaults(file = defaultsPath) {
  ...
  try {
    ...
    if (error.code === 'ENOENT' && file === defaultsPath) {
      try {
        text = await readFile(sharedVisionerDefaultsPath, 'utf8');
      } catch (fallbackError) {
        if (fallbackError.code === 'ENOENT') return {};
        throw Error('Cannot read shared Visioner live-test defaults');
      }
    } else if (error.code === 'ENOENT') return {};
    else throw Error('Cannot read local live-test defaults');
  }
  try {
    ...
    for (const role of ['gm', 'player']) {
      ...
      if (
        account.allowBlankPassword !== undefined &&
        typeof account.allowBlankPassword !== 'boolean'
      )
        throw Error();
    }
    return value;
  } catch {
    throw Error('Invalid local live-test defaults; expected JSON with account fields');
  }
}

export function accountDefaults(role, local = {}, env = process.env) {
  const prefix = `COMBATER_${role.toUpperCase()}`;
  ...
  const allowBlankPassword =
    role === 'player' &&
    (env.COMBATER_PLAYER_ALLOW_BLANK !== undefined
      ? env.COMBATER_PLAYER_ALLOW_BLANK === '1'
      : sameAccount && saved.allowBlankPassword === true);
  const password =
    env[`${prefix}_PASSWORD`] ??
    (sameAccount ? saved.password : undefined) ??
...
export async function promptAccount(role, local, env, prompts, context) {
  ...
  if (secret === undefined && role === 'player' && env.COMBATER_PLAYER_ALLOW_BLANK === '1')
    secret = '';
  if (secret === undefined) {
    const hint =
      selected.password === undefined
        ? ''
        : selected.password === ''
          ? ' (Enter uses blank password)'
          : ' (Enter keeps saved password)';
    const entered = await prompts.password(
      {
        message: `${role} password${hint}:`,
        mask: '*',
        toggleMask: false,
      },
      context,
  ...
  const allowBlankPassword =
    ...
    (env.COMBATER_PLAYER_ALLOW_BLANK === '1' ||
      (await prompts.confirm(
        {
          message: 'Confirm player account has no password.',
          default: selected.allowBlankPassword,
        },
        context,
      )));
