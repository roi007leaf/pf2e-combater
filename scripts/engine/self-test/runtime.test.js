...
assert.ok(
  executionTargetsSource.includes("matchesRecommendation ? null : source"),
  "execution targets should ignore old auto-stored recommendation targets unless marked manual",
);
assert.deepEqual(
...
try {
  ...
  const expelInfestation = classifySystemAction({
    name: "Expel Infestation",
    slug: "expel-infestation",
    description: {
      value: "<p>The isqulug expels larvae from the hivemind in its head in a @Template[cone|distance:30]. Creatures in this area take @Damage[6d10[piercing]] damage (@Check[reflex|dc:30|basic] save).</p>",
    },
  }, { actionCost: 2 });
...
const flankExpansion = builderAtomicActionsForStep({
  id: "flank-strike-claw",
  slug: "flank-strike-claw",
  name: "Stride to Flank (Claw's Reach)",
  actionCost: 2,
  item: { id: "claw" },
  traits: ["agile"],
...
// Reported live: the panel showed a bare "Stride" and a bare weapon Strike as two disconnected
// rows with no visible link between them, and the Strike's own MAP badge/target chip looked like
// they'd been slapped onto "just movement" -- both atoms must share one groupId/groupLabel (mirroring
// requiresBackingStrike composites like Sudden Charge) so the panel nests them under one header,
// with the composite's own 2-action cost paid once (on the Stride), not once per atom.
assert.equal(flankExpansion[0].groupId, flankExpansion[1].groupId, "flank's Stride and Strike atoms must share one groupId so the panel nests them under one header");
assert.ok(flankExpansion[0].groupId, "flank atoms must actually have a groupId, not just match on undefined");
assert.equal(flankExpansion[0].groupLabel, "Stride to Flank (Claw's Reach)");
assert.equal(flankExpansion[0].atomIndex, 0);
assert.equal(flankExpansion[1].atomIndex, 1);
assert.equal(flankExpansion[0].actionCost, 2, "the composite's full action cost belongs on the first (Stride) atom");
assert.equal(flankExpansion[1].actionCost, 0, "the second (Strike) atom must not double-charge the composite's action cost");
...
// Same reasoning as flank -- a skirmish's 3 atoms are one bundled activity, not three disconnected
// rows, so they must share one groupId/groupLabel with the composite's own cost paid once.
assert.equal(skirmishStrikeExpansion[0].groupId, skirmishStrikeExpansion[2].groupId, "skirmish's atoms must share one groupId so the panel nests them under one header");
assert.ok(skirmishStrikeExpansion[0].groupId, "skirmish atoms must actually have a groupId, not just match on undefined");
assert.deepEqual(skirmishStrikeExpansion.map((action) => action.actionCost), [3, 0, 0], "the composite's full action cost must land on the first atom only");
assert.deepEqual(skirmishStrikeExpansion.map((action) => action.atomIndex), [0, 1, 2]);
...
assert.equal(skirmishSpellExpansion[0].groupId, skirmishSpellExpansion[1].groupId, "a 2-atom skirmish must also be grouped under one header");
assert.deepEqual(skirmishSpellExpansion.map((action) => action.actionCost), [2, 0]);
...
assert.ok(/autoFillPanelDraft\([\s\S]*strideStepTowardPlannedTarget\(step, atomicSteps, index\)/.test(panelDraftWorkflowSource),
...
assert.ok(panelDraftWorkflowSource.includes("autoFillStrideOverSpeed"), "auto-fill should clamp stride destinations to the actor's Speed");
assert.ok(
  /function autoFillStrideOverSpeed[\s\S]*canvas\?\.grid\?\.measurePath[\s\S]*cost > speed/.test(panelDraftHelpersSource),
...
assert.ok(/autoFillPanelDraft\([\s\S]*filter\(\(step\) => !isRedundantAutoFillMove\(step\)\)/.test(panelDraftWorkflowSource),
  "auto-fill should skip redundant (negative-scored) generic basic moves");
assert.ok(/function isRedundantAutoFillMove\([\s\S]*Number\(step\?\.score\) < 0/.test(panelDraftHelpersSource),
...
assert.ok(/function autoFillAppliesProne\([\s\S]*includes\("drop-prone"\)/.test(panelDraftHelpersSource),
  "auto-fill prone detection should match a slug containing drop-prone");
assert.ok(/autoFillPanelDraft\([\s\S]*planAppliesProne[\s\S]*isBasicAutoFillMove/.test(panelDraftWorkflowSource),
...
assert.ok(
  /autoFillPanelDraft\([\s\S]*replacingDraft[\s\S]*buildCandidates\(panel\._context\)[\s\S]*bestTurnPlan\(panel\._context/.test(panelDraftWorkflowSource),
  "replacing an existing draft should rebuild Auto-fill from the real context, not the draft-projected context",
);
assert.ok(/function strideImprovesPosition\([\s\S]*before - after\) >= minGain/.test(panelDraftHelpersSource),
  "a kept Stride must improve position toward its target");
assert.ok(/autoFillPanelDraft\([\s\S]*!strideImprovesPosition\([\s\S]*return null/.test(panelDraftWorkflowSource),
...

{
  ...
  try {
    const makeActor = (id, name, type = "npc", actions = []) => ({
      id,
      uuid: `Actor.${id}`,
      name,
      type,
      ...
      isOwner: true,
      itemTypes: { condition: [], effect: [], action: [], feat: [], feature: [], spell: [], consumable: [], weapon: [] },
      items: [],
      getActiveTokens: () => [],
      system: {
        actions,
    ...
    const shortbow = {
      ...
      variants: [{ label: "MAP 0", roll: async () => null }],
    };
    const heroActor = makeActor("hero", "Hero", "character", [shortbow]);
    const oldActor = makeActor("old-foe", "Old Foe");
    const selectedActor = makeActor("selected-foe", "Selected Foe");
    ...
    const heroToken = makeToken("token-hero", "Hero", heroActor, 1, 0);
    const oldToken = makeToken("token-old", "Old Foe", oldActor, -1, 5);
    const selectedToken = makeToken("token-selected", "Selected Foe", selectedActor, -1, 10);
    ...
    const heroCombatant = makeCombatant(heroToken);
    globalThis.ui = { notifications: { warn: () => {}, info: () => {} } };
    globalThis.canvas = {
      grid: {
        size: 1,
        measurePath: ([from, to]) => Math.abs((to?.x ?? 0) - (from?.x ?? 0)),
      },
      tokens: {
        placeables: [heroToken, oldToken, selectedToken],
      },
    };
    globalThis.game = {
      user: { id: "gm", name: "GM", isGM: true, targets: new Set() },
      combat: {
        id: "combat-auto-fill-target",
        round: 1,
        turn: 0,
        started: true,
        combatant: heroCombatant,
        combatants: [heroCombatant, makeCombatant(oldToken), makeCombatant(selectedToken)],
      },
    };

    const staleContext = readCombatContext("auto-fill-stale-target-test", { combatant: heroCombatant });
    assert.equal(staleContext.battlefield.targets[0].name, "Old Foe");
    globalThis.game.user.targets = new Set([selectedToken]);

    const {
      atomizePanelAutoFillSteps: atomizeAutoFillSteps,
      autoFillPanelDraft: runAutoFillPanelDraft,
    } = await import("../../ui/panel/draft-workflow.js");
    let writtenDraft = null;
    const panel = {
      refreshSource: "auto-fill-live-target-test",
      _selectedCombatant: heroCombatant,
      _context: staleContext,
      _autoFillPlans: buildTurnPlans(staleContext, buildCandidates(staleContext).candidates),
      _builder: null,
      _plan: null,
      _pinnedPlanId: null,
      _autoFillInFlight: false,
      _canEditDraft: () => true,
      _readActiveDraftPlan: () => ({ steps: [], source: null }),
      _writeActiveDraftPlan: async (draft) => { writtenDraft = draft; },
      _atomizeAutoFillSteps(plan, context, prefixSteps = []) {
        return atomizeAutoFillSteps(this, plan, context, prefixSteps);
      },
      _actionKeyForStep: (step) => actionBuilderKey(step),
      render: async () => {},
    };

    await runAutoFillPanelDraft(panel);
    assert.ok(
      writtenDraft?.steps?.some((step) => step.targetTokenIds?.includes(selectedToken.id)),
      "Auto-fill should rebuild from currently selected canvas target, not stale panel target",
    );
    assert.equal(
      writtenDraft?.steps?.some((step) => step.targetTokenIds?.includes(oldToken.id)),
      false,
      "Auto-fill should not keep stale fallback target when a live target is selected before pressing Auto-fill",
    );
  } finally {
    globalThis.game = previousGame;
...

{
  ...
  const previousUi = globalThis.ui;
  try {
    const expelDescription = "<p>Expel Infestation affects @Template[cone|distance:30]. The creatures take @Damage[6d10[piercing]] damage with a @Check[reflex|dc:30|basic] save.</p>";
    const expelAction = {
      id: "expel-infestation",
      name: "Expel Infestation",
      label: "Expel Infestation",
      slug: "expel-infestation",
      type: "action",
      actionType: "action",
      actions: 2,
      description: { value: expelDescription },
      traits: [],
      item: {
        id: "expel-infestation-item",
        name: "Expel Infestation",
        slug: "expel-infestation",
        type: "action",
        system: {
    ...
    const isqulugActor = makeActor("isqulug", "Isqulug", "npc", [expelAction]);
    const ezrenActor = makeActor("ezren", "Ezren");
    const celdarActor = makeActor("celdar", "Celdar Stoneplow");
    const eastActor = makeActor("east-foe", "East Foe");
    const makeToken = (id, name, actor, disposition, x, y) => ({
      id,
      name,
      actor,
      x,
    ...
    const isqulugToken = makeToken("token-isqulug", "Isqulug", isqulugActor, -1, 0, 0);
    const ezrenToken = makeToken("token-ezren", "Ezren", ezrenActor, 1, 0, 200);
    const celdarToken = makeToken("token-celdar", "Celdar Stoneplow", celdarActor, 1, 200, 0);
    const eastToken = makeToken("token-east-foe", "East Foe", eastActor, 1, 300, 0);
    const makeCombatant = (token) => ({
      id: `combatant-${token.id}`,
      name: token.name,
      actor: token.actor,
    ...
    const isqulugCombatant = makeCombatant(isqulugToken);
    globalThis.ui = { notifications: { warn: () => {}, info: () => {} } };
    globalThis.canvas = {
      grid: {
        size: 100,
        measurePath: ([from, to]) => ({ distance: (Math.hypot((to?.x ?? 0) - (from?.x ?? 0), (to?.y ?? 0) - (from?.y ?? 0)) / 100) * 5 }),
      },
      scene: { grid: { distance: 5, size: 100 } },
      walls: {},
      tokens: {
        placeables: [isqulugToken, ezrenToken, celdarToken, eastToken],
      },
    };
    globalThis.game = {
      user: { id: "gm", name: "GM", isGM: true, targets: new Set() },
      combat: {
        id: "combat-auto-fill-area-target",
        round: 1,
        turn: 0,
        started: true,
        combatant: isqulugCombatant,
        combatants: [
          isqulugCombatant,
          makeCombatant(ezrenToken),
          makeCombatant(celdarToken),
          makeCombatant(eastToken),
        ],
      },
    };

    const staleContext = readCombatContext("auto-fill-stale-area-target-test", { combatant: isqulugCombatant });
    const staleExpel = buildCandidates(staleContext).candidates.find((candidate) => candidate.slug === "expel-infestation");
    assert.ok(staleExpel, "sanity check: generated Expel Infestation candidate exists");
    assert.deepEqual(
      staleExpel.activityProfile.areaPlacementAimPoint,
      { x: 250, y: 50 },
      "sanity check: stale unfocused area placement aims at the better east cluster",
    );
    const stalePlan = buildTurnPlans(staleContext, [staleExpel])
      .find((plan) => plan.steps.length === 1 && plan.steps[0].slug === "expel-infestation");
    assert.ok(stalePlan, "sanity check: stale Expel Infestation plan exists");
    globalThis.game.user.targets = new Set([ezrenToken]);

    const {
      atomizePanelAutoFillSteps: atomizeAutoFillSteps,
      autoFillPanelDraft: runAutoFillPanelDraft,
      cyclePanelAutoFillDraft: runCyclePanelAutoFillDraft,
      fillPanelDraftGap: runFillPanelDraftGap,
    } = await import("../../ui/panel/draft-workflow.js");
    let writtenDraft = null;
    const panel = {
      refreshSource: "auto-fill-selected-area-target-test",
      _selectedCombatant: isqulugCombatant,
      _context: staleContext,
      _autoFillPlans: [stalePlan],
      _builder: null,
      _plan: null,
      _pinnedPlanId: null,
      _pinnedFillPlanId: null,
      _autoFillInFlight: false,
      _canEditDraft: () => true,
      _readActiveDraftPlan: () => ({ steps: [], source: null }),
      _writeActiveDraftPlan: async (draft) => { writtenDraft = draft; },
      _activeAutoFillPlans() {
        return this._autoFillPlans.filter((plan) =>
          plan.steps.some((step) => step.slug === "expel-infestation"));
      },
      _activePinnedPlanId() {
        return this._pinnedPlanId;
      },
      _hasManualDraftContent: () => false,
      _autoFillDraft(options = {}) {
        return runAutoFillPanelDraft(this, options);
      },
      _atomizeAutoFillSteps(plan, context, prefixSteps = []) {
        return atomizeAutoFillSteps(this, plan, context, prefixSteps);
      },
    ...
    await runCyclePanelAutoFillDraft(panel);
    const expelStep = writtenDraft?.steps?.find((step) => step.name === "Expel Infestation");
    assert.ok(expelStep, `cycled Auto-fill should choose Expel Infestation in this regression, got ${(writtenDraft?.steps ?? []).map((step) => step.name).join(" -> ")}`);
    assert.equal(
      expelStep?.areaMarker?.rotation,
      90,
      "cycled Auto-fill should rebuild a stale area plan from the live selected target before storing the template",
    );

    const decoyPlan = {
      id: "decoy-fill-plan",
      steps: [{ id: "decoy-step", name: "Decoy", slug: "decoy", actionCost: 1, score: 999 }],
      totalCost: 1,
      score: 999,
      summary: "Decoy",
    };
    let fillGapDraft = null;
    const fillGapPanel = {
      ...panel,
      _autoFillInFlight: false,
      ...
      _fillGapPlans() {
        const focusedExpel = buildCandidates(this._planningContext).candidates.find((candidate) => candidate.slug === "expel-infestation");
        const focusedExpelPlan = buildTurnPlans(this._planningContext, [focusedExpel])
          .find((plan) => plan.steps.length === 1 && plan.steps[0].slug === "expel-infestation");
        return [decoyPlan, focusedExpelPlan];
      },
...
const straightMovementRoute = movementRouteForStep(baseRouteContext, {
  slug: "stride",
  destination: { x: 15, y: 0 },
}, { gridSize: 5 });
assert.equal(straightMovementRoute.enabled, true, "movement route module enables Stride routing");
assert.equal(straightMovementRoute.reachable, true, "straight route inside Speed is reachable");
assert.equal(straightMovementRoute.cost, 15, "straight route reports PF2e movement cost");
assert.deepEqual(straightMovementRoute.destination, { x: 15, y: 0 }, "straight route keeps destination plain");
assert.equal(straightMovementRoute.waypoints, undefined, "straight route needs no corner waypoints");

const blockedMovementRoute = movementRouteForStep(baseRouteContext, {
  slug: "stride",
  destination: { x: 10, y: 0 },
}, {
  gridSize: 5,
...
assert.equal(blockedMovementRoute.enabled, true);
assert.equal(blockedMovementRoute.reachable, false, "blocked route reports illegal destination");
assert.match(blockedMovementRoute.reason, /movement path/i);

const waypointCost = waypointPathCost(
  { x: 0, y: 0 },
  [{ x: 5, y: 0 }, { x: 5, y: 5 }],
  { gridSize: 5 },
);
assert.equal(waypointCost.cost, 10, "waypoint route sums segment costs");

const flyingMovementRoute = movementRouteForStep({
  token: {
    id: "route-fly-token",
    center: { x: 0, y: 0 },
    width: 1,
    ...
    actor: { system: { movement: { speeds: { fly: { value: 40 } } } } },
  },
  actor: { profile: { speed: 25 } },
}, {
  slug: "stride",
  movementAction: "fly",
  destination: { x: 15, y: 0, elevation: 10 },
}, { gridSize: 5 });
assert.equal(flyingMovementRoute.maxCost, 40, "fly route uses fly speed as budget");
assert.equal(flyingMovementRoute.cost, 25, "fly route includes vertical movement cost");
assert.equal(flyingMovementRoute.reachable, true, "fly route with horizontal plus vertical cost in Speed is reachable");

const occupiedMovementRoute = movementRouteForStep(baseRouteContext, {
  slug: "stride",
  destination: { x: 5, y: 0 },
}, {
  gridSize: 5,
  isOccupied: (center) => center.x === 5 && center.y === 0,
});
...
const teleportMovementRoute = movementRouteForStep(baseRouteContext, {
  ...
  targetingProfile: { maxRange: 30 },
  destination: { x: 35, y: 0 },
}, { gridSize: 5 });
assert.equal(teleportMovementRoute.enabled, true, "teleport routes use movement route module");
assert.equal(teleportMovementRoute.maxCost, 30, "teleport route uses max range as budget");
assert.equal(teleportMovementRoute.reachable, false, "teleport destination beyond max range is illegal");
assert.match(teleportMovementRoute.reason, /range/i);

const sharedReachableCenters = reachableMovementCenters({ x: 0, y: 0 }, 10, { gridSize: 5 });
assert.ok(
  sharedReachableCenters.some((center) => center.x === 10 && center.y === 0),
  "movement route module exposes reachable centers within Speed",
);
const blockedReachableCenters = reachableMovementCenters({ x: 0, y: 0 }, 10, {
  gridSize: 5,
  pathBlocked: (_from, to) => to.x === 5 && to.y === 0,
});
assert.equal(
  blockedReachableCenters.some((center) => center.x === 5 && center.y === 0),
  false,
  "movement route reachable centers honor collision adapter",
);
const occupiedReachableCenters = reachableMovementCenters({ x: 0, y: 0 }, 10, {
  gridSize: 5,
  context: {
    token: { center: { x: 0, y: 0 }, width: 1, height: 1 },
    battlefield: { allies: [{ token: { center: { x: 5, y: 0 }, width: 1, height: 1 } }] },
  },
});
assert.equal(
...
assert.equal(
  movementBudgetForStep({
    token: { actor: { system: { movement: { speeds: { land: { value: 35 } } } } } },
  }, { slug: "stride", movementAction: "walk" }),
  35,
  "movement route budget reads token land speed",
);
assert.equal(
  movementBudgetForStep({
...
assert.equal(
  ...
  40,
  "movement route budget falls back to action movement distance when no actor speed is available",
);
assert.deepEqual(
  movementRouteSegmentCost({ x: 0, y: 0 }, { x: 5, y: 5 }, { gridSize: 5 }),
  { cost: 5, diagonalCount: 1 },
  "movement route segment-cost helper exposes PF2e diagonal state",
);
assert.equal(
  ...
  25,
  "movement route horizontal budget subtracts planned vertical movement from typed Speed",
);
assert.equal(
  movementBudgetForStep({}, { slug: "translocate", activityProfile: { teleport: true } }, { teleportFallback: 1000 }),
  1000,
  "movement route budget can cap unknown-range teleports for native planning",
);
assert.deepEqual(
  movementOriginForContext({
    token: { plannedCenter: { x: 15, y: 20 }, center: { x: 5, y: 5 } },
  }, { gridSize: 5 }),
  { x: 15, y: 20 },
  "movement route origin prefers projected planned center",
);
assert.deepEqual(
  movementOriginForContext({
    token: { document: { x: 10, y: 20, width: 2, height: 1 } },
  }, { gridSize: 5 }),
  { x: 15, y: 22.5 },
  "movement route origin derives center from token document top-left and footprint",
);
assert.deepEqual(
  movementDestinationForStep({ destination: { x: 10, y: 15, elevation: 20 } }),
  { x: 10, y: 15, elevation: 20 },
  "movement route destination parser keeps optional elevation",
);
assert.deepEqual(
  movementWaypointsForStep({
    movementPlan: { waypoints: [{ x: 5, y: 0 }, { x: 5, y: 5 }] },
  }, { x: 10, y: 5 }),
  [{ x: 5, y: 0 }, { x: 5, y: 5 }, { x: 10, y: 5 }],
  "movement route waypoint parser appends the destination when the stored path does not already end there",
);
assert.deepEqual(
  movementPlanForWaypoints({
    token: {
      center: { x: 0, y: 0 },
      document: { elevation: 10 },
      actor: { system: { movement: { speeds: { fly: { value: 30 } } } } },
    },
  }, {
    slug: "stride",
    movementAction: "fly",
  }, [{ x: 5, y: 0, elevation: 15 }], { gridSize: 5 }),
  {
    native: false,
    waypoints: [{ x: 5, y: 0, elevation: 15 }],
    cost: 10,
    maxCost: 30,
  },
  "movement route waypoint plan counts vertical travel exactly once",
);
assert.deepEqual(
  movementPlanForDestination(baseRouteContext, { slug: "stride" }, { x: 10, y: 0 }, { gridSize: 5 }),
  {
    native: false,
    waypoints: [{ x: 10, y: 0 }],
    cost: 10,
  ...
  "movement route destination plan wraps a reachable route as a movement plan",
);
assert.deepEqual(
  movementFootprintForToken({ width: 100, height: 100, document: { width: 2, height: 3 } }),
  { widthCells: 2, heightCells: 3 },
  "movement route footprint helper prefers TokenDocument grid units over rendered token pixel size",
);
assert.deepEqual(
  movementPlacementForCenter({ x: 20, y: 30 }, { document: { width: 2, height: 3 }, width: 100, height: 100 }, 5),
  { center: { x: 20, y: 30 }, x: 15, y: 22.5, width: 10, height: 15 },
  "movement route placement helper converts token grid footprint to scene rectangle",
);
assert.deepEqual(
  movementFootprintCentersForToken({ x: 10, y: 10 }, { document: { width: 2, height: 1 } }, 5),
  [{ x: 7.5, y: 10 }, { x: 12.5, y: 10 }],
  "token geometry helper enumerates footprint cell centers from token center",
);
const previousCanvasTokenLookupCanvas = globalThis.canvas;
try {
  globalThis.canvas = {
    tokens: {
      placeables: [{
        id: "lookup-token",
        document: { id: "lookup-document-token", uuid: "Scene.Token.lookup-token" },
      }],
    },
  };
  assert.equal(
    canvasTokenById("lookup-token")?.id,
    "lookup-token",
    "canvas geometry token lookup resolves live canvas tokens by id",
  );
  assert.equal(
    canvasTokenById("lookup-document-token")?.id,
    "lookup-token",
    "canvas geometry token lookup resolves TokenDocument ids",
  );
  assert.equal(
    ["lookup-token"].map(canvasTokenById)[0]?.id,
    "lookup-token",
    "canvas geometry token lookup remains safe as an Array.map callback",
  );
  assert.equal(
    canvasTokenById("explicit-token", { placeables: [{ id: "explicit-token" }] })?.id,
    "explicit-token",
    "canvas geometry token lookup accepts explicit Foundry token collections",
  );
  globalThis.canvas = { tokens: new Map([["map-token", { id: "map-token" }]]) };
  assert.equal(
    canvasTokenById("map-token")?.id,
    "map-token",
    "canvas geometry token lookup accepts canvas token collections without placeables",
  );
} finally {
  globalThis.canvas = previousCanvasTokenLookupCanvas;
}
assert.equal(
  wallBlocksMovement({ document: { move: 0, c: [5, -5, 5, 5] } }),
  false,
  "canvas geometry open movement wall should not block movement",
);
assert.equal(
  wallBlocksLine({ document: { sight: 1, move: 0, c: [5, -5, 5, 5] } }),
  true,
  "canvas geometry line wall should block sight even when movement is open",
);
assert.equal(
  wallBlocksMovement({ document: { move: 1, door: 1, ds: 1, c: [5, -5, 5, 5] } }),
  false,
  "canvas geometry open door should not block movement",
);
assert.equal(
  isLockedDoorWall({ document: { door: 1, ds: 2, c: [5, -5, 5, 5] } }),
  true,
  "canvas geometry should identify locked door walls",
);
const blockingCanvasWall = { document: { move: 1, c: [5, -5, 5, 5] } };
assert.equal(
  wallSegmentsBlockMovement({ x: 0, y: 0 }, { x: 10, y: 0 }, { walls: [blockingCanvasWall] }),
  true,
  "canvas geometry wall segment fallback blocks crossing segments",
);
assert.equal(
  wallSegmentsBlockMovement({ x: 0, y: 10 }, { x: 10, y: 10 }, { walls: [blockingCanvasWall] }),
  false,
  "canvas geometry wall segment fallback ignores non-crossing segments",
);
let tokenCollisionCalled = false;
assert.equal(
  canvasMovementPathBlocked({ x: 0, y: 0 }, { x: 5, y: 0 }, {
    collisionToken: {
      checkCollision(destination, collisionOptions) {
        tokenCollisionCalled = destination.x === 5 && collisionOptions.origin.x === 0;
        return true;
      },
    },
  }),
  true,
  "canvas movement collision helper delegates token collision before wall fallback",
);
assert.equal(tokenCollisionCalled, true, "canvas movement collision helper passes destination and origin to token collision");
assert.equal(
  canvasAttackPathBlocked({ x: 0, y: 0 }, { x: 10, y: 0 }, { walls: [blockingCanvasWall] }),
  true,
  "canvas attack collision helper uses wall-segment fallback when Foundry collision layer is unavailable",
);
assert.equal(
  canvasLinePathBlocked({ x: 0, y: 0 }, { x: 10, y: 0 }, { walls: [{ document: { sight: 1, move: 0, c: [5, -5, 5, 5] } }] }),
  true,
  "canvas line collision helper treats sight-only walls as blocked lines",
);
const reachMetrics = { pixelSize: 5, pixelsPerFoot: 1 };
const reachOriginPlacement = { center: { x: 0, y: 0 }, x: -2.5, y: -2.5, width: 5, height: 5 };
const reachTargetPlacement = { center: { x: 10, y: 0 }, x: 7.5, y: -2.5, width: 5, height: 5 };
assert.equal(
  rectangleDistanceFeet(reachOriginPlacement, reachTargetPlacement, reachMetrics),
  5,
  "canvas geometry rectangle distance measures edge gap in feet",
);
assert.equal(
...
assert.equal(
  ...
  true,
  "canvas geometry perimeter reach succeeds when sampled rays are unblocked",
);
assert.equal(
  canReachPlacementPerimeter(reachOriginPlacement, reachTargetPlacement, 5, { pathBlocked: () => true }),
  false,
  "canvas geometry perimeter reach fails when every sampled ray is blocked",
);
assert.equal(
  pf2eTokenMovementActionForStep({ slug: "crawl" }),
  "crawl",
  "token movement action keeps crawl distinct for Foundry movement",
);
assert.equal(
  pf2eTokenMovementActionForStep({ slug: "step" }),
  "walk",
  "token movement action maps Step to Foundry walk movement",
);
assert.equal(
  pf2eTokenMovementActionForStep({ movementAction: "step" }),
  "walk",
  "explicit step movement action maps to Foundry walk movement",
);
assert.equal(
  pf2eTokenMovementActionForStep({ movementAction: "fly" }),
  "fly",
  "explicit typed movement action passes through for Foundry movement",
);
...
const plannedOriginStridePreview = movementPreviewForStep({
  token: { plannedCenter: { x: 15, y: 20 }, center: { x: 0, y: 0 } },
  actor: { profile: { speed: 25 } },
  battlefield: { targets: [] },
}, { slug: "stride" }, { gridSize: 5 });
assert.equal(plannedOriginStridePreview.enabled, true);
assert.deepEqual(
  plannedOriginStridePreview.origin,
  { x: 15, y: 20 },
  "movement preview should use movement route origin helper so planned centers win",
);

const documentOriginStridePreview = movementPreviewForStep({
  token: { document: { x: 10, y: 20, width: 2, height: 1 } },
  actor: { profile: { speed: 25 } },
  battlefield: { targets: [] },
}, { slug: "stride" }, { gridSize: 5 });
assert.equal(documentOriginStridePreview.enabled, true);
assert.deepEqual(
  documentOriginStridePreview.origin,
  { x: 15, y: 22.5 },
  "movement preview should derive origin from token document top-left through movement route",
);

const actionDistanceStridePreview = movementPreviewForStep({
  token: { center: { x: 0, y: 0 } },
  battlefield: { targets: [] },
}, {
  slug: "stride",
...
assert.equal(
  actionDistanceStridePreview.distanceFeet,
  40,
  "movement preview should use movement route budget rules for action-provided movement distance",
);
assert.ok(
  actionDistanceStridePreview.reachableCenters.some((center) => center.x === 40 && center.y === 0),
  "movement preview reachable grid should extend to the action-provided movement distance",
);
...
const occupiedTeleportPreview = movementPreviewForStep({
  token: { id: "token-tp", center: { x: 0, y: 0 }, width: 1, height: 1 },
  actor: { profile: { speed: 25 } },
  battlefield: {
    enemies: [{ id: "teleport-blocker", token: { center: { x: 15, y: 0 }, width: 1, height: 1 } }],
  },
}, {
  slug: "translocate",
  activityProfile: { teleport: true },
...
assert.equal(occupiedTeleportPreview.destinationAvailable, false, "teleport destination must be unoccupied");
assert.match(occupiedTeleportPreview.destinationIllegalReason, /occupied/i);
...
const occupiedExplicitMovementPreview = movementPreviewForStep({
  token: { id: "token-moving", center: { x: 0, y: 0 }, width: 1, height: 1 },
  actor: { profile: { speed: 25 } },
  battlefield: {
    enemies: [{ id: "blocker", token: { center: { x: 10, y: 0 }, width: 1, height: 1 } }],
  },
}, {
  slug: "stride",
  destination: { x: 10, y: 0 },
...
assert.equal(occupiedExplicitMovementPreview.destinationAvailable, false);
assert.match(occupiedExplicitMovementPreview.destinationIllegalReason, /occupied/i);
...
try {
  ...
  // A move-and-strike composite (Flank, skirmish, etc.) only ever showed its stride path on hover --
  // nothing communicated WHICH enemy the Strike at the end targets, so it read as a bare, unexplained
  // Stride (reported live: "why does it show a Stride overlay if there's no Stride involved", for a
  // Flank->Strike composite that DOES internally reposition before attacking). The same green target
  // highlight a plain Strike gets must also appear here, layered on top of the movement preview.
  actionPreviewCalls.length = 0;
  const flankLikePreview = showActionPreview({
    token: { id: "actor-token" },
    actor: { profile: { speed: 25 } },
  }, {
    name: "Flank -> Claw",
    ...
    activityProfile: {
      ...
      includesStrike: true,
      strideCount: 1,
      attackCenter: { x: 150, y: 100 },
    },
  });
  assert.equal(flankLikePreview?.type, "movement", "sanity: a move-and-strike composite should still report its movement preview");
  assert.ok(actionPreviewCalls.some((call) =>
    call.type === "drawRect" && call.x === 150 && call.y === 125 && call.width === 100 && call.height === 50),
    "hovering a move-and-strike composite must also highlight its resolved target, not only the stride path");
  ...
  actionPreviewCalls.length = 0;
  const computedAreaPreview = showActionPreview({
    token: { id: "actor-token", center: { x: 100, y: 100 } },
  }, {
    name: "Expel Infestation",
    slug: "expel-infestation",
    targetingProfile: { area: true, type: "cone", distance: 30 },
    activityProfile: { areaPlacementAimPoint: { x: 300, y: 100 } },
  });
  assert.equal(computedAreaPreview.type, "area");
  assert.equal(computedAreaPreview.marker.shape, "cone");
  assert.equal(computedAreaPreview.marker.rotation, 0);
  assert.ok(actionPreviewCalls.some((call) =>
    call.type === "arc" && call.x === 100 && call.y === 100 && call.radius === 300),
    "area hover preview should compute a planned NPC cone marker even before the draft stores areaMarker");
...
// A dynamically-slugged action that's still a genuine plain Stride (e.g. Flank's "flank-strike-*",
// which is not literally the "stride" slug) must still get the full destination-picker/reachable-
// area preview -- requiresDestination is the same canonical signal requiresDestinationForAction()
// already uses to decide whether the panel shows a "Select destination" button in the first place,
// so a step that gets that button must also get a working preview when the button is clicked.
const flankLikeMovementPreview = movementPreviewForStep({
  token: { center: { x: 0, y: 0 } },
  actor: { profile: { speed: 25 } },
  battlefield: {},
}, { slug: "flank-strike-claw", requiresDestination: true }, { gridSize: 5 });
assert.equal(flankLikeMovementPreview.enabled, true, "a non-standard-slug step with requiresDestination must still get a movement preview");

// Reported live: the pre-filled flank destination rendered as an unreachable red X even though it
// was well within the actor's actual Speed -- movementBudgetForStep only recognized the literal
// slugs "crawl"/"step"/"stride"/"stand-stride" and fell through to a budget of 0 for anything else,
// so ANY destination beyond the origin itself looked "beyond range" regardless of the real Speed.
const flankLikeDestinationPreview = movementPreviewForStep({
  token: { center: { x: 0, y: 0 } },
  actor: { profile: { speed: 25 } },
  battlefield: {},
}, { slug: "flank-strike-claw", requiresDestination: true, destination: { x: 20, y: 0 } }, { gridSize: 5 });
assert.equal(
  flankLikeDestinationPreview.destinationAvailable,
  true,
  "a flank-like step's pre-filled destination well within Speed must not be marked beyond range",
);
// Reported live: "let's also show distance label for that on the stride line like we have for
// regular stride" -- a single plain destination (no manually-placed waypoints) drew its trail line
// with no distance readout at all; only a multi-waypoint Stride's own segmentLabels got one. A
// single destination is just a one-segment path, so it gets exactly the same label treatment.
assert.equal(flankLikeDestinationPreview.stridePath?.length, 1, "sanity: a plain destination draws a single-entry stridePath");
assert.deepEqual(
  flankLikeDestinationPreview.stridePath[0].segmentLabels?.map((label) => label.text),
  ["20 ft"],
  "a plain single-destination Stride must show a distance label on its trail line, same as a multi-waypoint one",
);
...
const expelInfestationCandidate = {
  id: "generated-expel-infestation",
  name: "Expel Infestation",
  slug: "expel-infestation",
  actionCost: 2,
  source: "system-inferred",
  ...
  targetingProfile: { area: true, type: "cone", distance: 30 },
  saveProfile: { stat: "reflex", dc: 30, basic: true },
  damageProfile: { formula: "6d10", type: "piercing" },
};
const expelTarget = {
  id: "ezren-token",
  uuid: "Scene.Token.ezren-token",
  name: "Ezren",
  distance: 20,
  center: { x: 0, y: 100 },
  token: { id: "ezren-token", uuid: "Scene.Token.ezren-token", center: { x: 0, y: 100 } },
};
const expelScored = scoreCandidate({
  ...fighterContext,
  token: { id: "isqulug-token", center: { x: 0, y: 0 } },
  targets: [expelTarget],
  battlefield: { enemies: [expelTarget], targets: [expelTarget], allies: [] },
}, expelInfestationCandidate);
assert.deepEqual(
  expelScored.activityProfile.areaPlacementAimPoint,
  { x: 0, y: 100 },
  "an Expel Infestation-style NPC cone should aim at the selected enemy during scoring",
);
const expelMarker = computeAreaMarker({ token: { id: "isqulug-token", center: { x: 0, y: 0 } } }, expelScored);
assert.equal(expelMarker?.shape, "cone");
assert.deepEqual(expelMarker?.center, { x: 0, y: 0 });
assert.equal(expelMarker?.rotation, 90);
...
try {
  ...
  assert.ok(flankComposite, "expected a flank recommendation when an ally is adjacent to the target");
  // Reported live: "we should consider flank as regular stride (it just needs to show which strike
  // name has that reach and add the strike traits)" -- Flank is a genuine, plain 1-action Stride,
  // not a bundled Stride+Strike composite. It must NOT set includesStrike/positionalTactic/
  // meleeStrike (those atomize it into separate sub-actions and mark it an attack for MAP purposes)
  // -- that's exactly what previously produced a MAP badge and no destination-picker button on
  // something that's just movement. Carrying the strike's own item keeps the trait chips
  // (Agile/Reach/Unarmed) so the player still sees WHICH strike's reach makes the square work.
  assert.equal(flankComposite.actionCost, 1);
  assert.equal(flankComposite.attackTrait, false, "flank must not read as an attack step (no MAP badge)");
  assert.equal(flankComposite.requiresDestination, true, "flank must get the normal destination-picker button, like any other Stride");
  assert.equal(requiresTargetForAction(flankComposite), false, "flank is a plain Stride -- its destination is already computed from the flank square, so no target-select control is needed");
  assert.equal(flankComposite.activityProfile.includesStrike, undefined, "flank must not be treated as a move-and-strike composite");
  assert.equal(flankComposite.activityProfile.positionalTactic, undefined, "flank must not be atomized into separate Stride/Strike sub-actions");
  assert.equal(flankComposite.activityProfile.setsUpFlank, true, "flank keeps its own marker for scoring's GM-preference bonus, distinct from positionalTactic");
  assert.equal(flankComposite.item?.id, "claw", "flank must still carry the strike's own item so its trait chips (Agile/Reach/Unarmed) display");
  assert.equal(flankComposite.name, "Stride to Flank (Claw's Reach)");
  ...
  assert.equal(flankComposite.activityProfile.flankAllyName, "Valeros", "activityProfile should carry the ally's name so scoring can name them in the tooltip");
  ...
  // Reported live: "auto fill should auto set move destination though" -- Auto-fill's own
  // presetDestination logic (CombaterPanel.js's _atomizeAutoFillSteps) only ever reads a step's
  // top-level `destination` field, never activityProfile.attackCenter -- without this, Auto-fill
  // added the flank Stride with no destination at all, forcing "Choose destination at execution."
  // even though the correct square was already computed right here.
  assert.deepEqual(flankComposite.destination, flankSquare, "flank must expose its computed square as a top-level destination so Auto-fill can pre-fill it");
  ...
  // A single creature cannot flank alone -- the composite is only ever offered with a real ally
  // threatening the target (asserted above), but the SCORE reason must actually say so too, or a
  // player sees "Flanks Goblin for an off-guard Strike" with no mention of the ally and reasonably
  // assumes the tool thinks a lone attacker can flank by itself.
  const flankScored = scoreCandidate({ ...flankContext, isGM: true }, flankComposite);
  assert.ok(
    flankScored.reason.includes("Valeros"),
    `the flank score reason must name the ally that makes the flank legal, got "${flankScored.reason}"`,
  );
  // Reported live: "it's not clear that it's a STRIDE using [the strike's] reach for the flank, it
  // looks like it wants to flank and attack with tentacles" -- the reader's own default reason
  // already says "Stride to flank...", but the score's own reason (which wins the tooltip) dropped
  // that verb entirely, reading as a single atomic "Flanks X for a Strike" with no movement implied.
  assert.ok(
    /stride/i.test(flankScored.reason),
    `the flank score reason must make clear a Stride repositions the actor first, got "${flankScored.reason}"`,
  );
  // Follow-up: naming the ally/Stride still read as "wants to attack with the strike" rather than
  // explaining WHY that destination square works -- it's within the strike's own reach, which is
  // what lets the actor flank without necessarily closing to bare 5ft adjacency.
  assert.ok(
    /\d+-foot reach/i.test(flankScored.reason),
    `the flank score reason must explain that the strike's reach is what makes the flanking square work, got "${flankScored.reason}"`,
  );
} finally {
  globalThis.canvas = previousFlankCanvas;
}

// Large (2x2+) targets: the flanking square must be genuinely across from the ally's own row/
// column, not merely anywhere on the opposite side of the target's CENTER -- reported live with
// exact coordinates (Isqulug at document (2500,1000), Calder Stoneplow ally at (2700,1200), Gromog
// target 2x2 at document (2800,1100), gridSize 100): the tool offered a flank square one row off
// from Calder's own row (the row directly opposite Calder's adjacent corner of the 2x2 target),
// which a center-point-only check accepts just as readily as the genuinely opposite square.
const previousLargeTargetFlankCanvas = globalThis.canvas;
try {
  globalThis.canvas = {
    scene: { grid: { distance: 5 } },
    grid: { size: 100 },
  ...
  const largeTargetFlankContext = {
    ...
    token: { id: "isqulug-token", center: { x: 2550, y: 1050 } },
    profile: { speed: 30, reach: 5, conditions: { slugs: [], values: {} }, skills: {} },
    battlefield: {
      allies: [{ id: "calder", name: "Calder Stoneplow", token: { center: { x: 2750, y: 1250 } } }],
      enemies: [{
        id: "gromog", name: "Gromog", distance: 20,
        token: { center: { x: 2900, y: 1200 }, document: { width: 2, height: 2 } },
      }],
      targets: [{
        id: "gromog", name: "Gromog", distance: 20,
        token: { center: { x: 2900, y: 1200 }, document: { width: 2, height: 2 } },
  ...
  const largeTargetFlank = readActionSources(largeTargetFlankContext).find((action) => action.slug === "flank-strike-claw");
  assert.ok(largeTargetFlank, "expected a flank composite against the Large target");
  const largeTargetFlankSquare = largeTargetFlank.activityProfile.attackCenter;
  // Calder is adjacent to Gromog's south-west cell (its own row's y-center is 1250) -- the correct
  // flank square must be on the opposite (east) side AND share that same row, not the row one cell
  // north of it (y-center 1150), which the old center-point-only check accepted just as readily.
  assert.equal(largeTargetFlankSquare.y, 1250, `flank square must share the ally's own row against a Large target, got ${JSON.stringify(largeTargetFlankSquare)}`);
  assert.equal(largeTargetFlankSquare.x, 3050, `flank square must be on the opposite side of the Large target, got ${JSON.stringify(largeTargetFlankSquare)}`);
} finally {
  globalThis.canvas = previousLargeTargetFlankCanvas;
}
...
try {
  ...
  assert.ok(
    gmFlankScore.reasons.some((reason) => reason.includes("Strides within 5-foot reach to flank")),
    "GM flank score should cite the off-guard flank advantage",
  );
  assert.ok(
    !playerFlankScore.reasons.some((reason) => reason.includes("Strides within 5-foot reach to flank")),
...
function scoreCandidateWithActorStrikes(context, action) {
  const actor = context?.actor?.document ?? context?.combatant?.actor ?? context?.actor ?? null;
  return scoreCandidate(context, action, [], actorStrikeOptions(actor, context));
}

const backingStrikeScored = scoreCandidateWithActorStrikes(backingStrikeContext, doubleAttackForBackingStrike);
...
const ordinaryScoredForBackingStrike = scoreCandidateWithActorStrikes(backingStrikeContext, ordinaryCandidateForBackingStrike);
...
const suddenChargeScored = scoreCandidateWithActorStrikes(suddenChargeBackingStrikeContext, suddenChargeForBackingStrike);
...
const ordinaryBackingStrikeScored = scoreCandidateWithActorStrikes(suddenChargeBackingStrikeContext, ordinaryActionForBackingStrikeGate);
...
const flyingKickScored = scoreCandidateWithActorStrikes(flyingKickBackingStrikeContext, flyingKickForBackingStrike);
...
const flurryScored = scoreCandidateWithActorStrikes(flurryBackingStrikeContext, flurryOfBlowsForBackingStrike);
...
assert.equal(grabManeuverScored.slug, "grapple", "Grab must resolve to the real Grapple action by slug so the system action execution lookup finds it");
