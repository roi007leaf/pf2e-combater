...
function readRule() {
  try {
    const rule = setting(SETTINGS.flankingSizeRule);
    return Object.values(FLANKING_SIZE_RULES).includes(rule) ? rule : FLANKING_SIZE_RULES.raw;
  } catch {
...
export function registerFlankingSizeRuleWrapper(libWrapperAdapter = globalThis.libWrapper) {
  ...
  if (typeof libWrapperAdapter?.register === 'function') {
    ...
    const patched = Symbol.for('pf2e-combater.flankingSizeRulePatched');
    if (proto[patched]) return true;
    const original = proto.onOppositeSides;
    proto.onOppositeSides = function (flanker, ally, target) {
      return wrapOnOppositeSides(original.bind(this), flanker, ally, target);
    };
    proto[patched] = true;
  }
  return true;
